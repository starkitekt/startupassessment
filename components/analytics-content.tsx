...
import { DatePickerWithRange } from "@/components/ui/date-range-picker" // Assume this component exists
...
const analyticsChartConfig = {
  ...
  avgValuation: { label: "Avg Valuation (Cr)", color: "hsl(var(--chart-2))" },
  jobsCreated: { label: "Jobs Created", color: "hsl(var(--chart-1))" },
  revenueGenerated: { label: "Revenue (Cr)", color: "hsl(var(--chart-2))" },
}
      ...
      <Tabs defaultValue="portfolioHealth" className="w-full">
        ...
        <TabsContent value="portfolioHealth" className="mt-6 space-y-6">
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            {/* Add more cards for valuation trends, KPI benchmarks etc. */}
            <Card>
              <CardHeader>
                <CardTitle>Valuation vs. Risk</CardTitle>
                <CardDescription>Startup valuation against risk score.</CardDescription>
              </CardHeader>
              <CardContent>
                <ChartContainer config={analyticsChartConfig} className="h-[250px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                      <CartesianGrid />
                      <XAxis type="number" dataKey="avgValuation" name="Avg Valuation (Cr)" unit="Cr" />
              ...
              <ChartContainer config={analyticsChartConfig} className="h-[350px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={funnelData}>
                    ...
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Legend />
                    <Bar dataKey="count" name="Applications at Stage" fill="var(--color-count)" radius={[4, 4, 0, 0]} />
                    {/* Could add another bar for conversion rate if data is structured for it */}
                  </BarChart>
                </ResponsiveContainer>
              </ChartContainer>
            </CardContent>
          </Card>
          {/* Add cards for time per stage, drop-off reasons */}
        </TabsContent>
        ...
        <TabsContent value="impactAnalysis" className="mt-6 space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
                ...
                <ChartContainer config={analyticsChartConfig} className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={impactData}>
                      ...
                      <ChartTooltip content={<ChartTooltipContent />} />
                      <Legend />
                      <Line type="monotone" dataKey="jobsCreated" stroke="var(--color-jobsCreated)" strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
                </ChartContainer>
              </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Total revenue generated by portfolio companies (in Cr).</CardDescription>
              </CardHeader>
              <CardContent>
                <ChartContainer config={analyticsChartConfig} className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={impactData}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="year" />
                      <YAxis dataKey="revenueGenerated" unit="Cr" />
                      <ChartTooltip content={<ChartTooltipContent />} />
                      <Legend />
                      <Bar dataKey="revenueGenerated" fill="var(--color-revenueGenerated)" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </ChartContainer>
              </CardContent>
          ...
          {/* Add card for geographical impact (Map or regional bar chart) */}
        </TabsContent>
      </Tabs>
    </div>
  )
...
// Mock DatePickerWithRange if not already defined elsewhere
